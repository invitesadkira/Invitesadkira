...
async function renderGuestView() {
  ...
  // ✅ Questionário "O que achou deste convite?" foi removido a pedido —
  // já não aparece em lado nenhum, nem no evento de exemplo.
  ...
  try {
    ...
    if (eventData && eventData.length > 0) {
      ...
      event_color: event.event_color || null,
        ...
        confirmations: (event.rsvps || []).filter(r => !r.is_manual_ticket).map(rsvp => ({
