...
function saveEventWithCover(eventId, title, date, time, deadline, coverImageURL, allowComp, maxComp, allowGifts, allowKids, maxKids, allowSides, side1Name, side2Name, allowMessages, showGuestMessages, musicUrl, musicTitle, ibanMessage, ibanNumber, ibanHolder, ibanFooter, vis, submitBtn, originalText) {
  ...
  const eventData = {
    ...
    show_time: document.getElementById('sw-show-time').classList.contains('active') ? 'yes' : 'no',
        ...
        external_rsvp_url: (document.getElementById('evt-external-rsvp-url') || document.querySelector('.evt-external-rsvp-url-all'))?.value?.trim() || null,
    ...
    if (result) {
      ...
      if (typeof saveEventVisuals !== 'undefined') {
        saveEventVisuals(eventId, {
          ...
          custom_text_show_invite: document.getElementById('evt-custom-text-invite')?.checked ? 'yes' : 'no',
          ...
          story_date_color: document.getElementById('evt-story-date-color-hex')?.value?.trim() || null,
...
function _fillEditForm(ev) {
  ...
  { const v = ev.external_rsvp_url || ''; const eu = document.getElementById('evt-external-rsvp-url'); if (eu) eu.value = v; document.querySelectorAll('.evt-external-rsvp-url-all').forEach(i => i.value = v); }
  ...
  { const bgC = document.getElementById('evt-bg-color'); const bgH = document.getElementById('evt-bg-color-hex'); const v = ev.bg_color || ''; if (bgH) bgH.value = v; if (bgC) bgC.value = v && /^#[0-9a-fA-F]{6}$/.test(v) ? v : '#ffffff'; }
  ...
  { const ctInv = document.getElementById('evt-custom-text-invite'); if (ctInv) ctInv.checked = ev.custom_text_show_invite !== 'no'; }
  ...
  { const cv=document.getElementById('evt-couple-video-url'); if(cv)cv.value=ev.couple_video_url||'';
    if(ev.couple_video_url){
      _setSwitch('sw-couple-video', true, 'couple-video-extra');
      const isYt = ev.couple_video_url.includes('youtube.com') || ev.couple_video_url.includes('youtu.be');
      if(isYt){
        const r=document.getElementById('cvt-youtube'); if(r){r.checked=true; window._toggleCoupleVideoType?.();}
        const yu=document.getElementById('evt-couple-video-yt-url'); if(yu)yu.value=ev.couple_video_url;
      } else {
        const vp=document.getElementById('couple-video-preview'); const vw=document.getElementById('couple-video-preview-wrap');
        if(vp)vp.src=ev.couple_video_url; if(vw)vw.style.display='';
      }
    }
    const vam=ev.video_audio_mode||'pause_music'; const r=document.getElementById('video-audio-'+(vam==='replace_music'?'replace-music':'pause-music')); if(r)r.checked=true; }
...
async function openIntakeFormMain(eventId) {
  ...
  const result = await supabaseRequest(
    `events?id=eq.${eventId}&select=id,title,date,time,confirm_by_date,cover_image,groom_name,bride_name,music_url,iban_number,iban_holder,iban_number_2,iban_holder_2,bible_text,bible_ref,groom_parents,bride_parents,gallery_urls,invite_text,story_text,schedule_items,show_schedule&limit=1`
  );
