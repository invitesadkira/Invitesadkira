  ...
  supabaseRequest(`events?id=eq.${eventId}`, 'PATCH', {
    ...
    if (result) {
      if (ev) {
        ...
        ev.invite_blessing = document.getElementById('evt-invite-blessing')?.value.trim() || null;
...
function renderEventDetails() {
  ...
  if (confirmations.length === 0) {
    ...
    confContainer.innerHTML = confirmations.map((c, idx) => {
      ...
      const reservedGift = event.gifts && event.gifts.find(g => g.reserved && normalizeGuestName(g.reservedBy) === guestKey);
      ...
      if (isOwner) {
        ...
        if (hasReservedGift) {
          actionButtons += '<button class="text-yellow-500 hover:text-yellow-600 transition p-1" title="Remover presente" onclick="removeGiftReservation(\'' + reservedGift.id + '\')"><i data-lucide="gift" class="w-4 h-4"></i></button>';
        }
      ...
      if (hasReservedGift && isOwner) {
        removeGiftBtn = '<button class="text-yellow-500 hover:text-yellow-600 transition p-1" title="Remover presente reservado" onclick="removeGiftReservation(\'' + reservedGift.id + '\')"><i data-lucide="gift" class="w-5 h-5"></i></button>';
      }
      let replyMessageBtn = '';
...
async function searchEvent() {
  ...
  try {
    ...
    const allEvents = await supabaseRequest(`events?select=id,title,date,time,confirm_by_date,cover_image,event_code,allow_companions,max_companions,allow_gifts,allow_kids,max_kids,allow_sides,side1_name,side2_name,show_time,rsvp_enabled,save_the_date_enabled,release_type,release_date,is_invite_released,std_title,std_subtitle,std_font_family,std_name_size,std_title_size,std_intro_enabled,std_intro_text,std_intro_photo_url,std_show_cover,std_cover_url,std_scratch_enabled,std_scratch_mode,std_scratch_photo_url,std_scratch_text,personalized_links_enabled,show_rsvp_in_full_invite,show_guest_name_in_invite,allow_messages,show_guest_messages,rsvps(guest_name,attending,side,companions,kids,wants_gift,message,created_at,updated_at),gifts(id,name,category,reserved,reserved_by)`);
  ...
  if (eventCode) {
    ...
    if (eventsData && eventsData.length > 0) {
      ...
      const normalizedEvent = {
        ...
        invite_blessing: eventData.invite_blessing || null,
