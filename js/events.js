    ...
    if (result) {
      ...
      if (typeof saveEventVisuals !== 'undefined') {
        saveEventVisuals(eventId, {
          ...
          couple_video_url: document.getElementById('evt-couple-video-url')?.value || null,
...
function _fillEditForm(ev) {
  ...
  { const cv=document.getElementById('evt-couple-video-url'); if(cv)cv.value=ev.couple_video_url||'';
    if(ev.couple_video_url){ const vp=document.getElementById('couple-video-preview'); const vw=document.getElementById('couple-video-preview-wrap'); if(vp)vp.src=ev.couple_video_url; if(vw)vw.style.display=''; }
